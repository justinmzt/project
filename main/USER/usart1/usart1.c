...
{
	u8 rec_data;delay_init();	
	if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET)  //接收中断 
		{
				rec_data =(u8)USART_ReceiveData(USART1);         //(USART1->DR) 读取接收到的数据
        if(receive_str[0]==0x00) uart_byte_count=1;	  
			  if((uart_byte_count>0)&&(uart_byte_count<=128))
				{
				   receive_str[uart_byte_count-1]=rec_data;
				   uart_byte_count++;
				}   
				else if(uart_byte_count>128)
				{
           uart1SendChars((char *)receive_str,128);
					 for(uart_byte_count=0;uart_byte_count<128;uart_byte_count++)receive_str[uart_byte_count]=0x00;
					 receive_str[0]=rec_data;
					 uart_byte_count=2; 
				}
