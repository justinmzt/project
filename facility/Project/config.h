#ifndef __CONFIG_H
#define __CONFIG_H
#include "stdio.h"
#include "stdbool.h"
#include "common.h"
//////////////////////////////////////////////////////////////////////////////////


u16 a[78]={0};   //智能配电柜遥测量（设备号1）
u16 sf[6]={0x35,0x01,0x03,0x01}; //智能配电柜遥信量
u16 b[296]={1,2};   //锂电池遥测量
u16 c[24]={0};   //锂电池遥信量
u16 d[55]={0};   //UPS遥测量
u16 e[103]={0};   //UPS遥信量
/** 站号:01
* ele[1]~ele[77]=====add[1]~add[77]
*/
u16 ele[78]={0};
/** 站号:02
* ups1[1]~ups1[112]=====add[1]~add[113]
* ups1[113]~ups1[145]=====add[1001]~add[1033]
* ups1[146]~ups1[289]=====add[1101]~add[1244]
* ups1[290]~ups1[433]=====add[1501]~add[1644]
* ups1[434]~ups1[517]=====add[1901]~add[1984]
* ups1[518]~ups1[520]=====add[2301]~add[2303]
*/
u16 ups1[521]={1};
/** 站号:03
* ups2[1]~ups2[112]=====add[1]~add[113]
* ups2[113]~ups2[145]=====add[1001]~add[1033]
* ups2[146]~ups2[289]=====add[1101]~add[1244]
* ups2[290]~ups2[433]=====add[1501]~add[1644]
* ups2[434]~ups2[517]=====add[1901]~add[1984]
* ups2[518]~ups2[520]=====add[2301]~add[2303]
*/
u16 ups2[521]={0};
u16 ele2[5] = {0};
u16 ele3[2] = {0};
/*****16位CRC检验表，低位在前，高位在后*******/
//////////////*高位表*///////////////////
u8 auchCRCHi[] = {

} ;

//////////////*低位表*///////////////////
u8 auchCRCLo[] = {

} ;


u16 disp[8]={0,0,0,0,0,0,0,0};					//字接收显示数组，用于存储6x1-6x8的十六位接收数据
u8 count=0;															//收到数据的次数
u8 time=0;															//应用于发送03代码时，中间的字数量是多少个数值

u16 switch_address=0;										//位操作MODBUS地址（01,05使用）
u16 swh=0;															//位操作存放的组数
//u16 data_address=0;	
u8 signF,signE,signD,signC,signB,signA,	//用于判定发送的16个位，即0x1-0xF的位状态
sign9,sign8,sign7,sign6,sign5,sign4,sign3,
sign2,sign1,sign0;		


/*设备相关*/
u8 flag_hmi_send=0;												//设备响应中断接收数据位数
u8 flag_finish=0;												//设备响应中断接收数据位数
u8 flag_byte_count=8;										//是否接收完成位（各设备串口共用）
u16 flag_add=1;													//设备响应中断接收地址
#define USART1_BYTE_COUNT 			8  	//定义usart1接收端接收长度
#define USART_REC_NUM  			200  	//定义最大接收字节数 200
#define USART_SEND_NUM  			200  	//定义最大接收字节数 200
u8 uart_byte_count;          //uart_byte_count要小于USART_REC_LEN
u8 receive_str[USART1_BYTE_COUNT];
u8 use_str[USART1_BYTE_COUNT];
u8 receive_str2[USART_REC_NUM];
u8 Send_buf[USART_SEND_NUM]; 						//发送数据数组
u16  address_num=0;				   						//接收到字的数值
u16  begin_address=0;										//提取接收缓存数组得到地址开始位
u16  address_leg=0;											//提取返回数值长度
/*load相关*/
u8 location = 1;
u8 load_uart_num[] = {1,1,1,2,2,2,2,2,2,2,2,2,3,3,3,3,3,3,3,3,3};
u8 load_func_code[] = {1,2,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3};
u16 load_add[] = {0,0,0,1,81,1901,1001,1101,1173,1501,1573,2301,1,81,1901,1001,1101,1173,1501,1573,2301};
u8 load_qua[] = {16,40,78,55,32,84,34,72,72,72,72,3,55,32,84,34,72,72,72,72,3};
#endif